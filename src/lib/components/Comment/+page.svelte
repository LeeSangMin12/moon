<script>
	...
	let are_replies_visible = $state(true);
	...
	const handle_add_reply = async () => {
		if ($user_store.id && reply_content.trim()) {
	...
	<div class="flex w-full gap-3">
		...
		<div class="w-full">
			<div class="mb-0.5 flex items-center gap-2">
				<span class="text-sm font-medium text-black">{comment.users.name}</span>
			...
			{#if is_reply_open}
				<div class="mt-2 flex items-center">
					<textarea
						...
						class="flex-1 resize-none rounded-lg bg-gray-100 p-2 text-sm focus:outline-none"
					></textarea>
					<button
			...
			{#if comment.replies && comment.replies.length > 0}
				<button
					class="mt-4 flex items-center text-xs text-blue-500 hover:underline"
